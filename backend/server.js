...
const {ValidateCreateUser} = require('./Services/User_Service')
...
.then(async () => {
    ...
    //Users
    await ValidateCreateUser(11710616, 'Rebeccalyn', 'Lao', 'Reb', 'password', "reb.lao@dlsu.edu.ph", 'EDUCATION', "Who was your childhood hero?", "Harry Styles")
    await ValidateCreateUser(11726059, 'Denzel', 'Co', 'Denz', 'password', "den.co@dlsu.edu.ph", 'MANAGER', "Who was your childhood hero?", "Barney")
    await ValidateCreateUser(11710111, 'Schuyler', 'Ng', 'Sky', 'password', "johnny.appleseed@dlsu.edu.ph", 'EDUCATION', "Who was your childhood hero?", "Steve Jobs")

    //Book Instance
    db.book_instance.create({bookinstance_id: 1, status: 1, book_id: 1})
    .then()
    .catch(err=>console.log)
    db.book_instance.create({bookinstance_id: 2, status: 1, book_id: 1})
    .then()
    .catch(err=>console.log)
    db.book_instance.create({bookinstance_id: 3, status: 1, book_id: 2})
    .then()
    .catch(err=>console.log)
    db.book_instance.create({bookinstance_id: 4, status: 1, book_id: 2})
    .then()
    .catch(err=>console.log)
    db.book_instance.create({bookinstance_id: 5, status: 1, book_id: 3})
    .then()
    .catch(err=>console.log)
    db.book_instance.create({bookinstance_id: 6, status: 1, book_id: 3})
    .then()
    .catch(err=>console.log)
