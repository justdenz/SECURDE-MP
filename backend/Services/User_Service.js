...
const {Register, Login, LogChangePassword} = require("./UserAction_DB.js")
...
async function ValidateCreateUser(user_id, first_name, last_name, username, password, email, role_name, question, answer){
    let response = {
        status: '',
        payload: ''
    }
...
async function ValidateChangePassword(user_id, new_password){
    ...
    if(result == 1){
        ...
        await LogChangePassword(user_id)
        .then()
        .catch(err => console.log(err))
    } else {
        response.status = "ERROR"
